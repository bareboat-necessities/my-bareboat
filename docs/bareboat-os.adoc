...
https://github.com/bareboat-necessities/lysmarine_gen/releases/tag/v2022-03-13

Past LTS release:

https://github.com/bareboat-necessities/lysmarine_gen/releases/tag/v2022-03-06
...
NOTE: Before doing it, go into Disks tool from App Menu/Utilities/Disks and remove all partitions from your SSD drive.
...
- Are you using pi4, CM4 (with wifi), or pi400?
...
There are reports that PC Windows software for WIB2/3 can be run on Raspberry Pi with with Box86 and Wine:
...
- Interfaces (USB, Ethernet, CAN Bus, Rs-485, RS-232, WiFi, 4G/LTE, GPS/GNSS, ModBus, etc)
...
Zigbee operates on short distances 10-100m in about same spectrum range as WiFi with lower
...
Z Wave is similar to Zigbee but uses different RF spectrum which doesn't clash with WiFi.
...
There is a script to install various additional WiFi cards drivers
...
=== Setting up multiple WiFi access points

For seamless switching for WiFi client from one access point to another you can set up all access points
with the same SSID and credentials on different radio frequiency channels.
...
* 10 points capacitive touch screen monitor,
* 1000nit high brightness, -20 to 70 degree wide use temperature.
* industrial front IP65 waterproof dust-proof vehicle use aluminum alloy bezel style.
...
- Internet connectivity on your boat via WiFi at marinas or via LTE and a local boat WiFi hotspot is an important feature to have
- For off-shore sailors with SSB radio experience consider what you can have with open source solutions on Pi
