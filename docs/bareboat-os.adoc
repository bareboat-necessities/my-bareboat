...
VHF and AIS use similar frequencies, need be in direct sight of view, so the higher they are the better range.
They are limited by Earth curvature. Range is around 30 miles. They can use the same antenna with a proper splitter.
Transmitter on AIS has to be licensed.
...
Transmitting on SSB (Single Side Band) radio requires a license for an operator.
