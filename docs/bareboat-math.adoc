...
Although leeway calculation is not required for the algorithm (it gets taked into account with using
heading to course over ground difference), for completeness here is a popular empirical formula:
...
stem:[\text{accel}_\text{earthXYZ} = q * \text{accel} * q'], where stem:[q'] is inverted quaternion stem:[q].
...
stem:[ Q = [[ \text{PosIntegral_Trans_Variance}, 0, 0 \], [0, 0.2, 0 \], [0, 0, 0.1 \]\] ]
