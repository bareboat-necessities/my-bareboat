= Documentação do Sistema Operacional do Bareboat Necessities.
mgrouch <mgrouch@users.noreply.github.com>
{docdate}, Documentação do Sistema Operacional do Bareboat Necessities
:imagesdir: images
:doctype: book
...
Eletrônicos náuticos comerciais, tablets inteligentes e multiplexadores Wi-Fi não substituem um computador de bordo,
componente importante capaz de monitorar, armazenar dados e controlar todos os sistemas de uma embarcação.
Com o BBN-OS instalado em um computador de bordo baseado em Raspberry Pi (Pi) de baixo consumo de energia, ao contrário
de computadores ou notebooks, atende suas necessidades com flexibilidade para adicionar inúmeras opções de sensores
e se comunicar com todos os sistemas da embarcação tais como: Internet, Wi-Fi local, câmeras, rede NMEA e um sistema
capaz de decodificar protocolos de rádio marítimos.
O BBN-OS é gratuito baseado em projetos de código aberto suportados pela comunidade e comumente usados, como SignalK,
PyPilot, OpenCPN e outros. A interface gráfica do usuário do BBN-OS permitirá que você crie uma estação no cockpit
para todas as funcionalidades do sistema operacional, desde gráficos, painéis, informações meteorológicas, reprodutor
de mídia, etc.
...
* Inicie o OpenCPN. Vá em Ferramentas / Opções / Gráficos / guia Downloader de gráficos.
* Clique em 'Adicionar Catálogo'. Para os EUA: clique em USA NOAA & Inland Charts / ENC / By Region.
* Escolha sua região, clique (ou toque) em 'OK'
* Clique em 'Atualizar' (para atualizar o catálogo)
* Clique na guia 'Baixar gráficos ...'
* Clique com o botão direito (ou toque longo) na lista de gráficos
* Clique em 'Selecionar tudo' no menu pop-up
* Pressione o botão 'Baixar gráficos selecionados' e aguarde a conclusão
* Pressione o botão 'Aplicar'
* Clique na guia 'Arquivos de gráfico'
* Pressione o botão 'Prepare all ENC Charts'
* Pressione o botão 'OK' quando terminar.

O-Charts podem ser registrados usando plug-ins OpenCPN pré-instalados (no espaço do usuário arm32, certifique-se de atualizar os plug-ins on-line
do catálogo OpenCPN) com chave ou dongle USB O-Charts.

===== O-Charts

O-Charts são gráficos comerciais compatíveis com OpenCPN. Veja: https://www.o-charts.org/

===== Fazendo seus próprios gráficos MbTiles

Fazendo seus próprios gráficos MbTiles com SASPlanet:
http://svocelot.com/Cruise_Info/Equipment/mbTiles.htm

==== AvNav

Quando você está online, os blocos de MB raster NOAA devem funcionar imediatamente. O-Charts podem ser registrados usando o plugin AvNav
com chave ou dongle USB O-Charts.

==== SignalK, FreeBoard-SK, TukTuk

Siga a documentação do SignalK para instalar gráficos offline para eles.

=== Configure seus parâmetros da embarcação

Configure os parâmetros do seu navio nas aplicações SignalK e Vessel.
Eles podem ser úteis.

No plugin OpenCPN Polar, escolha um arquivo polar para a sua embarcação em ~/Polars,
/usr/share/opencpn/plugins/weather_routing_pi/data/polars/
ou construa a partir de suas gravações de dados de viagens anteriores. Será necessário para a rota do tempo, etc.

Os polares são importantes para os plug-ins Weather Routing e Dashboard Tactics.
