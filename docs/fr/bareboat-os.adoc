= Documentation pour BBN OS (Bareboat Necessities)

:imagesdir: images
:keywords: openplotter, opencpn, signalK, nmea, marine
:description: BBN Marine OS is a free open source Linux for Raspberry Pi on boats. BBN OS permet de construire un traceur, soit un ordinateur de bateau basé sur OpenCPN, SignalK et NMEA, similaire à OpenPlotter
:doctype: book
...
=== Pourquoi utiliser BBN OS ?

Pourquoi utiliser BBN OS ? Certaines personnes n'utilisent que de l'électronique marine commerciale à bord, d'autres utilisent des tablettes intelligentes et des multiplexeurs Wi-Fi marins. Toutes ces solutions manquent d'un composant important - un ordinateur embarqué capable de surveiller et de contrôler tous les aspects des systèmes de bateau, tout en étant capable de stocker des données. Certaines personnes utilisent un ordinateur basé sur les processeurs Intel.
Ces solutions basées consomment cependant plus d'énergie. Avec BBN OS, vous pouvez construire un ordinateur de bateau central répondant à tous vos besoins.
Entièrement basé sur Raspberry Pi, ce qui implique une faible consommation d'énergie, tout en ayant la souplesse pour ajouter d'innombrables choix de capteurs et de communiquer avec tous les systèmes de bateaux, Internet, Wi-Fi local, caméras, réseau NMEA et tout autre système capable de décoder les protocoles radio maritimes.
Initialement, ce projet a commencé à partir d'expérimentations visant à améliorer l'exprience untilisateur d'OpenPlotter.
BBN OS est gratuit et open source. Il est basé sur des projets open source généralement supportés par la communauté tels que SignalK,
PyPilot, OpenCPN et autres. L'interface utilisateur graphique de BBN OS vous permettra de construire un front-end dans le cockpit pour toutes les fonctionnalités de l'OS allant du traceur aux tableaux de bord, en passant par les informations météo, les lecteurs multimédias et bien d'autres.
...
Starting with BBN Marine OS version 2022-11-10 AIS-Catcher https://github.com/jvde-github/AIS-catcher is included too.
