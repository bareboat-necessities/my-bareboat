...
===== GL-X750 Router Modifications

You get this router with the modem you selected for your region. My modem is Quectel EP06-A.
EP06-A has 3 antenna connectors. One is connected to internal GPS antenna, another one to internal
LTE antenna and middle one is Rx Diversity antenna is not connected. You can open up router
by unscrewing two screws under serial number sticker (it's easy to remove and put back after).
I've just put another coax cable to Rx Diversity antenna connector and cut a bit of plastic in the case just
enough to put that cable through (SMA side on outside).

.GL-X750 Antenna Mod
image::gl-x750.jpg[alt="GL-X750 Antenna Mod"]

.GL-X750 Case Mod
image::gl-x750-antenna-mod.jpg[alt="GL-X750 Screws"]


Probably I should have put another external connector for GPS antenna too. From my understanding if this modem
loses GPS fix for long time it will lose LTE/4G/3G connection too. It was probably better to have both GPS and LTE
external antennas. Also I think before first use it needs to acquire GPS fix so I need to take this router outside
plugged in into power for an hour or so.
...
Long range WiFi antenna can go to one of two existing external SMA connectors.
