...
This document describes a DIY project of building a boat computer and cockpit display
from easily obtainable components. The computer will include number of sensors for:
...
The computer will run on Raspberry Pi with OpenPlotter installed. The computer will reside below deck so
it's not going to be waterproof. The computer will be connected to a waterproof HDMI touchscreen in
...
Front panel with have heavy duty panel mount interfaces for:
...
* 2 NMEA 0183 ports
* DC power cords mounted inside waterproof glands for 12v boat and cockpit connections
* HDMI and USB 3.0 connectors to cockpit display and touchscreen
* RF connector for AIS antenna
* RF connector for RTL-SDR antenna
* RF connector for secondary GPS
