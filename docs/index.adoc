...
Change desktop background color to black (Right Click on desktop -> Desktop Preferences).
...
Click 'Desktops' tab, add new desktop

Click 'Margins' tab, set all margins to 0
...
'Edit'/'Preferences'
Check 'Don't ask options on lauch executable'
...
curl -O https://github.com/Stellarium/stellarium/releases/download/v0.20.0/stellarium-0.20.0.tar.gz
gzip -cd < stellarium-0.20.0.tar.gz | tar xvf -
cd stellarium-0.20.0
mkdir -p builds/unix
cd builds/unix
cmake ../../
make -j4
...
----
cd /home/pi
curl https://www.meltemus.com/index.php/en/download/send/9-raspberrypi/203-qtvlm-5-9-8-6 > qtVlm-5.9.8-rpi.tar.gz
gzip -cd < qtVlm-5.9.8-rpi.tar.gz | tar xvf -
